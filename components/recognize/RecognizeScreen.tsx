          ...
          return (
            <>
              {currentRewards.map((r: any) => (
                <div 
                  key={r.id} 
                  style={{ opacity: r.stock === 0 ? 0.6 : 1, pointerEvents: r.stock === 0 ? 'none' : 'auto' }}
                >
                  <RewardCard 
                    title={r.title} 
                    points={r.points} 
              ...
              {/* Empty State */}
              {rewards.length === 0 && (
                <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: 20, color: HP_TOKENS.inkMute, border: `1.5px dashed ${HP_TOKENS.line}`, borderRadius: 20 }}>
