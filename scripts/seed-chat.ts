...
async function seedChat() {
  ...
  try {
    ...
    await conn.query(`INSERT INTO message_channels (id, name, type, avatar_emoji) VALUES (?, 'DM Mgr HR', 'dm', '')`, [dm2Id]);
    await conn.query(`INSERT INTO message_channel_members (channel_id, user_id) VALUES (?, ?), (?, ?)`, 
      [dm2Id, mgrId, dm2Id, hrId]);

    await conn.query(`INSERT INTO messages (id, channel_id, sender_id, content, message_type, created_at) VALUES 
      (?, ?, ?, 'Halo Ibu Maya, saya ingin berkonsultasi terkait prosedur pengajuan klaim lembur untuk tim saya minggu ini.', 'text', DATE_SUB(NOW(), INTERVAL 2 DAY)),
      (?, ?, ?, 'Halo Pak Budi. Untuk klaim lembur, silakan mengisi form e-Claim yang ada di portal HRD paling lambat tanggal 25.', 'text', DATE_SUB(NOW(), INTERVAL 1 DAY)),
      (?, ?, ?, 'Baik, terima kasih atas informasinya Ibu Maya.', 'text', NOW())`,
      [
        `msg_${uuidv4()}`, dm2Id, mgrId,
